...
| forceValidateOnBlur | boolean | false | The component does not validate if the input value is a number if the prop `format` is used. This forces the component to validate the value even if `format` is used |
